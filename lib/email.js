...
// Resend SDK v4 returns { data, error } instead of throwing — unwrap and throw on error.
...
// Sent to P1 when a 2-player session expires without P2 submitting.
// p2Email is included so P1 knows who didn't respond — no secrets revealed.
async function sendExpiryNoticeTwoPlayer({ p1Email, p2Email, title, p1SubmittedAt }) {
  await send({
    from: FROM,
    to: p1Email,
    subject: `Session expired — ${title}`,
    text: [
      `Your Secret Keeper session "${title}" has expired without a response.`,
      '',
      `You submitted your secret on ${isoTs(p1SubmittedAt)}.`,
      `${p2Email} did not submit before the 7-day deadline.`,
      '',
      `No secrets have been revealed. If you'd like to try again, create a new session at ${baseUrl()}.`,
      `You may reuse the same secret — nothing was shared.`,
      footer(),
    ].join('\n'),
  });
}

// Sent to all participants who submitted when a multi-player session expires.
// Shows who responded and who didn't — no secrets revealed.
async function sendExpiryNoticeMulti({ participants, title }) {
  const submitted = participants.filter(p => p.submitted);
  const waiting   = participants.filter(p => !p.submitted);

  const respondedLines = submitted.map(p => `  • ${p.email}`).join('\n');
  const waitingLines   = waiting.map(p => `  • ${p.email}`).join('\n');

  const body = [
    `Your Secret Keeper session "${title}" has expired without all players submitting.`,
    '',
    `Responded (${submitted.length}):`,
    respondedLines || '  (none)',
    '',
    `Did not respond (${waiting.length}):`,
    waitingLines || '  (none)',
    '',
    `No secrets have been revealed. If you'd like to try again, create a new session at ${baseUrl()}.`,
    `You may reuse the same secrets — nothing was shared.`,
    footer(),
  ].join('\n');

  const sends = submitted.map(p =>
    send({
      from: FROM,
      to: p.email,
      subject: `Session expired — ${title}`,
...
module.exports = {
  ...
  sendExpiryNoticeTwoPlayer,
  sendExpiryNoticeMulti,
};
