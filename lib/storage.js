'use strict';

const { neon, neonConfig, Pool } = require('@neondatabase/serverless');
const { randomBytes } = require('crypto');

neonConfig.fetchConnectionCache = true;

function getDb() {
  return neon(process.env.DATABASE_URL);
}

async function createSession({ title, p1Secret, p1Email, p2Email, observerEmails, notifyObservers }) {
  const p2Token = randomBytes(32).toString('hex');
  const sql = getDb();
  const rows = await sql`
    INSERT INTO sessions
      (title, p1_secret, p1_email, p2_email, observer_emails, notify_observers, p2_token)
    VALUES
      (${title}, ${p1Secret}, ${p1Email}, ${p2Email}, ${observerEmails}, ${notifyObservers}, ${p2Token})
    RETURNING *
  `;
  return rows[0];
}

async function getSessionByP2Token(p2Token) {
  const sql = getDb();
  const rows = await sql`
    SELECT id, title, p2_email, expires_at
    FROM sessions
    WHERE p2_token = ${p2Token}
      AND status = 'awaiting'
      AND expires_at > NOW()
  `;
  return rows[0] ?? null;
}

async function revealSession({ p2Token, p2Secret, p2Email }) {
  const pool = new Pool({ connectionString: process.env.DATABASE_URL });
  const client = await pool.connect();
  try {
    await client.query('BEGIN');

    const { rows } = await client.query(
      `SELECT * FROM sessions WHERE p2_token = $1 FOR UPDATE`,
      [p2Token]
    );

    if (rows.length === 0) {
      await client.query('ROLLBACK');
      return { error: 'not_found' };
    }

    const session = rows[0];

    if (session.status !== 'awaiting') {
      await client.query('ROLLBACK');
      return { error: 'already_used' };
    }

    if (new Date(session.expires_at) < new Date()) {
      await client.query('ROLLBACK');
      return { error: 'expired' };
    }

    const logToken = randomBytes(32).toString('hex');

    const { rows: updated } = await client.query(
      `UPDATE sessions
       SET status          = 'revealed',
           p2_secret       = $1,
           p2_email        = $2,
           p2_submitted_at = NOW(),
           revealed_at     = NOW(),
           log_token       = $3,
           log_expires_at  = NOW() + INTERVAL '14 days'
       WHERE id = $4
       RETURNING *`,
      [p2Secret, p2Email, logToken, session.id]
    );

    await client.query('COMMIT');
    return { session: updated[0] };
  } catch (err) {
    await client.query('ROLLBACK');
    throw err;
  } finally {
    client.release();
    await pool.end();
  }
}

async function getSessionByLogToken(logToken) {
  const sql = getDb();
  const rows = await sql`
    SELECT title, p1_email, p2_email, p1_secret, p2_secret,
           p1_submitted_at, p2_submitted_at, revealed_at
    FROM sessions
    WHERE log_token = ${logToken}
      AND status = 'revealed'
      AND log_expires_at > NOW()
  `;
  return rows[0] ?? null;
}

async function getRevealByP2Token(p2Token) {
  const sql = getDb();
  const rows = await sql`
    SELECT title, p1_email, p2_email, p1_secret, p2_secret,
           p1_submitted_at, p2_submitted_at, log_token
    FROM sessions
    WHERE p2_token = ${p2Token}
      AND status = 'revealed'
      AND log_expires_at > NOW()
  `;
  return rows[0] ?? null;
}

async function deleteSession(id) {
  const sql = getDb();
  await sql`DELETE FROM sessions WHERE id = ${id}`;
}

// Claims expired awaiting sessions for notification — returns the claimed rows so the cron
// can email submitters and then delete. Using DELETE ... RETURNING so each row is processed
// exactly once even if the cron fires concurrently.
async function claimExpiredSessions() {
  const sql = getDb();
  return sql`
    DELETE FROM sessions
    WHERE status = 'awaiting' AND expires_at < NOW()
    RETURNING title, p1_email, p1_submitted_at, p2_email
  `;
}

async function cleanupExpired() {
  const sql = getDb();
  // Only delete revealed sessions past retention; cron handles awaiting-expired (with notification email).
  await sql`
    DELETE FROM sessions
    WHERE status = 'revealed' AND log_expires_at < NOW()
  `;
}

module.exports = {
  createSession,
  deleteSession,
  claimExpiredSessions,
  getSessionByP2Token,
  getRevealByP2Token,
  revealSession,
  getSessionByLogToken,
  cleanupExpired,
};
